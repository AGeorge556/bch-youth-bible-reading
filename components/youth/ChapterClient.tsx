    ...
    <div className="space-y-6">
      {posterUrl && (
        <div>
          ...
          <p className="text-xs text-center text-muted-foreground mt-1">{t('tap_to_view_full')}</p>
        </div>
      )}
      ...
      {questions.length > 0 && (
        <div className="rounded-xl bg-card border border-border p-5 space-y-4">
          <h2 className="font-semibold">{t('reflection_questions')}</h2>
          {questions.map((q, i) => (
            <div key={q.id} className="space-y-1">
              <label className="text-sm font-medium">{i + 1}. {q.question_text}</label>
              <textarea value={answers[q.id] ?? ''} onChange={e => setAnswers(a => ({ ...a, [q.id]: e.target.value }))}
                rows={3} placeholder={t('your_answer')}
          ...
          <button onClick={handleSubmitAnswers} disabled={saving}
            ...
            {saving ? t('saving') : hasSubmitted ? t('save_changes') : t('submit_answers')}
          </button>
        </div>
      )}
    </div>
